...
#第一次要运行的：
#git clone git@github.com:sw586/uploadfile.git
...
git add .  &&  git commit -m "`date "+%Y-%m-%d_%H-%M-%S"`"  &&  git push -u origin +master  &&  curl "http://2ht.cn/index.php?s=githubup&c=home&m=index"